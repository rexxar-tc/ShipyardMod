...
namespace ShipyardMod.Utility
{
    public class Communication
    {
        ...
        private static byte[] MESSAGE_HASH = {90, 183}; //not actually a hash, just bitwise NOT of MESSAGE_ID, broken into bytes
        ...
        {
            byte[] data = MyAPIGateway.Utilities.SerializeToBinary(toolLine);
        ...
        public static void SendNewYard(ShipyardItem item, ulong steamId = 0)
        {
            ...
            byte[] data = MyAPIGateway.Utilities.SerializeToBinary(newYard);
        ...
        {
            byte[] messageBytes = MyAPIGateway.Utilities.SerializeToBinary(settings);
        ...
        {
            var item = MyAPIGateway.Utilities.SerializeFromBinary<ToolLineStruct>(data);
        ...
        private static void HandleNewShipyard(byte[] data)
        {
            ...
            var yardStruct = MyAPIGateway.Utilities.SerializeFromBinary<YardStruct>(data);
            ...
            ShipyardCore.TrackingYards.Add(yardItem);
        }
        ...
        {
            var msg = MyAPIGateway.Utilities.SerializeFromBinary<DialogStruct>(data);

            MyAPIGateway.Utilities.ShowMissionScreen(msg.Title, "", msg.Subtitle, msg.Message.Replace("|", "\r\n"), null, msg.ButtonText);
        }
        ...
        private static void HandleShipyardSettings(byte[] data)
        {
            ...
            byte[] setData = new byte[data.Length - sizeof(long)];
            Array.Copy(data, sizeof(long), setData, 0, setData.Length);
            var settings = MyAPIGateway.Utilities.SerializeFromBinary<YardSettingsStruct>(setData);
