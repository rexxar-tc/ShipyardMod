        ...
        public override void Handle()
        {
            foreach (ShipyardItem item in ProcessShipyardDetection.ShipyardsList)
            {
                ...
                if (item.YardGrids.Count == 0
                    || (item.YardType == ShipyardType.Grind && item.YardGrids.All(g => g.Physics == null))
                                         ...
                                         foreach (IMyCubeBlock welder in shipyardItem.Tools)
                                         {
                                             ...
                                             foreach (BlockTarget target in shipyardItem.BlocksToProcess[tool.EntityId])
                                             {
                                                 ...
                                                 if (!MyAPIGateway.Session.CreativeMode)
                                                 {
                                                     ...
                                                     {
                                                         targetsToRemove.Add(target);
                                                         if (shipyardItem.Settings.FillInventory && target.ProjectedBlock?.FatBlock != null)
                                                         {
                                                             var pfat = target.ProjectedBlock.FatBlock;
                                                             var fat = target.Block.FatBlock;
                                                             if (pfat.HasInventory)
                                                             {
                                                                 for (int k = 0; k < pfat.InventoryCount; k++)
                                                                 {
                                                                     MyInventory inv = (MyInventory)fat.GetInventory(k);
                                                                     MyInventory pinv = (MyInventory)pfat.GetInventory(k);
                                                                     if (pinv.Empty())
                                                                         continue;
                                                                     var items = pinv.GetItems();
                                                                     var itemsD = new Dictionary<string, int>();
                                                                     var itemsDe = new Dictionary<string, int>();
                                                                     foreach (var item in items)
                                                                     {
                                                                         int v;
                                                                         itemsD.TryGetValue(item.Content.SubtypeName, out v);
                                                                         v += (int)item.Amount;
                                                                         itemsD[item.Content.SubtypeName] = v;

                                                                         v = 0;
                                                                         itemsDe.TryGetValue(item.Content.SubtypeName, out v);
                                                                         itemsDe[item.Content.SubtypeName] = (int)((double)item.Amount * efficiency);
                                                                     }

                                                                     //move waste into temp inventory, then discard
                                                                     _tmpInventory.PullAny(shipyardItem.ConnectedCargo, itemsDe);
                                                                     _tmpInventory.Clear();
                                                                     //move items into target inventory
                                                                     inv.PullAny(shipyardItem.ConnectedCargo, itemsD);
